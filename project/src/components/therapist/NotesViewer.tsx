...
export const NotesViewer: React.FC<NotesViewerProps> = ({ open, onOpenChange }) => {
  ...
  const [noteDates, setNoteDates] = React.useState<Date[]>([]);
  ...
  // Fetch all dates that have notes for calendar highlighting
  React.useEffect(() => {
    fetchNoteDates();
  }, [user, open]);
  ...
  const fetchNoteDates = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/notes/dates/all', {
        headers: {
      ...
      if (response.ok) {
        ...
        setNoteDates(dates.map((dateStr: string) => new Date(dateStr)));
      }
    } catch (error) {
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-5 h-full">
                    ...
                    <div className="lg:col-span-2 p-6">
                      <DayPicker
                        ...
                        modifiers={{ hasNotes: noteDates }}
                        modifiersClassNames={{
                          hasNotes: 'border-2 border-violet-500 rounded-lg bg-violet-50 dark:bg-violet-900/20',
                        }}
                        classNames={{
                          ...
                          nav_button: 'h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 text-slate-600 dark:text-slate-400',
                          ...
                          day: 'h-9 w-9 p-0 font-normal aria-selected:opacity-100 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-700 dark:text-slate-300',
                          day_selected: 'bg-violet-600 text-white hover:bg-violet-700 focus:bg-violet-600',
                          day_today: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-semibold',
                    ...
                    {/* Notes Panel */}
                    <div className="lg:col-span-3 border-l border-slate-200/50 dark:border-slate-700/50 bg-gradient-to-br from-slate-50/50 to-white/50 dark:from-slate-800/50 dark:to-slate-900/50">
                            ...
                            <div>
                              ...
                              <p className="text-slate-600 dark:text-slate-400 text-sm">
                                ...
                                <span className="text-xs mt-1 block">
                                  Dates with notes are highlighted in violet
                                </span>
                              </p>
                            </div>
                          </div>
